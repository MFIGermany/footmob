...
export class BillingModel {
  ...
  normalizeStatus(status) {
    ...
    if (['canceled', 'cancelled', 'unpaid', 'incomplete_expired', 'past_due', 'paused'].includes(value)) {
      return 'inactive'
    }
    ...
    if (installId) {
      ...
      await pool.execute(sql, [
        ...
        normalizedStatus === 'active'
          ? this.unixToMysqlDate(Math.floor(Date.now() / 1000))
          : null,
        eventType,
  ...
  async getStatusByInstallId(installId) {
    ...
    const [rows] = await pool.execute(sql, [installId])
    return rows[0] || null
  }
}